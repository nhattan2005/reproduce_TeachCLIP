...
class CLIP4ClipPreTrainedModel(PreTrainedModel, nn.Module):
    ...
    def from_pretrained(cls, cross_model_name, state_dict=None, cache_dir=None, type_vocab_size=2, *inputs, **kwargs):
        ...
        # print(list(clip_state_dict.keys()))
        # 当前模型X-CLIP的state_dict 存储在 state_dict
            ...
            if contain_frame_position is False:
                for key, val in clip_state_dict.items():
                    ...
                    if model.sim_header == "seqTransf" and key.find("transformer.resblocks") == 0:
                        ...
                        if num_layer < 2:
                            state_dict[key.replace("transformer.", "frametransformer.")] = val.clone()
                        continue
...
class CLIP4Clip(CLIP4ClipPreTrainedModel):
    def __init__(self, cross_config, clip_state_dict, task_config):
        ...
        self.frameLinear = nn.Linear(transformer_width,transformer_width)
        self.frameLinear2 = nn.Linear(transformer_width,1)
    ...
    def forward(self, input_ids, token_type_ids, attention_mask, video, video_mask=None,return_fine=False):
        ...
        if self.training:
            # loss = 0.
            if return_fine:
                sim_matrix, _, Frameweight = self.get_similarity_logits(sequence_output, visual_output, attention_mask, video_mask,
                                                    shaped=True, loose_type=self.loose_type,return_fine=True)
                Frameweight = Frameweight.squeeze(-1)
                return sim_matrix,Frameweight
            else:
    ...
    def get_sequence_output(self, input_ids, token_type_ids, attention_mask, shaped=False):
        ...
        sequence_hidden = self.clip.encode_text(input_ids, return_hidden=False).float()
    ...
    def _loose_similarity(self, sequence_output, visual_output, attention_mask, video_mask, sim_header="meanP",return_fine=False):
        ...
        if self.training:
            ...
            if return_fine:
                Frameweight = allgather(Frameweight, self.task_config) # B F 1
            torch.distributed.barrier()

        visual_output = visual_output / visual_output.norm(dim=-1, keepdim=True)
        visual_output_mean = self._mean_pooling_for_similarity_visual(visual_output, video_mask)
        visual_output_mean = visual_output_mean / visual_output_mean.norm(dim=-1, keepdim=True)
        ...
        if return_fine:
            ...
            retrieve_logits_fine = logit_scale * torch.matmul(sequence_output, visual_output_adapt.t())
            retrieve_logits = retrieve_logits_fine

            return retrieve_logits, Frameweight.softmax(1)
        else:
            retrieve_logits = logit_scale * torch.matmul(sequence_output, visual_output_mean.t())
    ...
    def get_similarity_logits(self, sequence_output, visual_output, attention_mask, video_mask, shaped=False, loose_type=False, return_fine=False):
        ...
        if loose_type:
            ...
            if not return_fine:
                ...
                retrieve_logits,Frameweight = self._loose_similarity(sequence_output, visual_output, attention_mask, video_mask, sim_header=self.sim_header,return_fine=True)
                return retrieve_logits, contrastive_direction, Frameweight
