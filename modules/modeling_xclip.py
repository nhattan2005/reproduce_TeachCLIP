"""
Adapted from: https://github.com/xuguohai/X-CLIP/blob/main/modules/modeling_xclip.py
"""
...
class XCLIP(CLIP4ClipPreTrainedModel):
    ...
    def forward(self, input_ids, token_type_ids, attention_mask, video, video_mask=None,return_feat=False,return_fine=False):
        ...
        if self.training:
            # loss = 0.
            sim_matrix, _ = self.get_similarity_logits(sequence_output, seq_features, visual_output, attention_mask, 
                                        video_mask, shaped=True, loose_type=self.loose_type)
            return sim_matrix
        else:
            # loss = 0.
            if return_fine:
                sim_matrix, _,sentence2frames_sim= self.get_similarity_logits(sequence_output, seq_features, visual_output, attention_mask, 
                                        video_mask, shaped=True, loose_type=self.loose_type,return_fine=True)
                return sim_matrix,sentence2frames_sim
            else:
                sim_matrix, _ = self.get_similarity_logits(sequence_output, seq_features, visual_output, attention_mask, 
                                            video_mask, shaped=True, loose_type=self.loose_type)
                if return_feat:
                    return sim_matrix,[sequence_output,seq_features,visual_output]
                else:
                    return sim_matrix
    ...
    def _loose_similarity(self, sequence_output, seq_features, visual_output, attention_mask, video_mask, sim_header="meanP",return_fine=False):
        """
        ...
        if return_fine:
            sentence2frames_sim = torch.softmax(torch.matmul(sentence_output, frame_features.permute(0, 2, 1)), dim=-1) #BD BDF -->BBF  # / 1e-2
            sentence2frames_sim = torch.diagonal(sentence2frames_sim).T #BF
            return logits, sentence2frames_sim
        else:
            return logits
    ...
    def get_similarity_logits(self, sequence_output, seq_features, visual_output, attention_mask, video_mask, shaped=False, loose_type=False,return_fine=False):
        if shaped is False:
            attention_mask = attention_mask.view(-1, attention_mask.shape[-1])
            video_mask = video_mask.view(-1, video_mask.shape[-1])
        ...
        if loose_type:
            ...
            if return_fine:
                retrieve_logits,sentence2frames_sim = self._loose_similarity(sequence_output, seq_features, visual_output, attention_mask, video_mask, sim_header=self.sim_header,return_fine=True)
            else:
        ...
        if return_fine:
            return retrieve_logits, contrastive_direction,sentence2frames_sim
        else:
